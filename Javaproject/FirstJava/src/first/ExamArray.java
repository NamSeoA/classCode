...
public class ExamArray {
	...
	public static void main(String[] args) {  //main 메서드는 프로그램 시작 시점. 흐름, 인스턴스, 메서드를 만들어냄
...
﻿	*/
	...
	public static void changeArr(int[][] arr) {
		...
		int[] temp = new int[arr[0].length];   //0에 arr.length-1로 해도 됨
		
		temp = arr[arr.length-1];  //N-1  2차원 배열의 마지막 요소인 1차원 배열 temp에 저장
...
﻿*/
	...
	public static int miniValue(int[] arr) {  // 3, 2, 7, 5
		int min = arr[0];
		for(int i=1; i<arr.length; i++) { //두번째부터 비교하면 되기 때문에 1을 넣은 것
			if(min>arr[i]) {
				min=arr[i];
			}
		}
