...
public class PhoneBookManager implements Util {
		...
		final PhoneInfor[] phoneBook;  // 정보 저장을 위한 배열 선언,  변경이 되는 위험을 없애기 위해 상수처리
		
		int cnt;                 //저장된 정보의 개수 : 반복의 횟수, 
		                         //배열의 새로운 데이터 입력시 index로도 사용 가능,
		                         //삭제 시 빈칸이 생겼을 때 shift(데이터삭제)도 가능, cnt-1도 가능
...
//			sc = new Scanner(System.in);
		...
		void insertMember() { //사용자로부터 데이터를 받아서 매개변수 넣지않음
...
//			c++;  // 추가된 정부의 개수를 +1
		...
		// 정보 검색
		// 검색하고자하는 이름을 사용자로 부터 받고
		// 배열의 요소(참조값->인스턴스 참조가능) PhoneInfor.name 변수의 값과 검색 이름을 비교(동등비교)
		// 해당 index를 찾을 수 있다. 
		// 검색된 index로 정보 출력
		void searchMember() {
...
//			int index = -1;			
			...
			// 검색 : index를 찾는다.
//			for(int i=0; i<cnt; i++) {
//				if(phoneBook[i].name.equals(name)) { //현재 입력한 0번째의 이름과 문자열이 같은지 확인 후 boolean논리값을 가져온다.
//					index=i;
//					break;
//				}
//			}	
...
//		}	
			...
			if(index<0) {
				...
				// 삭제 처리 : 삭제해야하는 index부터 왼쪽으로 shift 처리한다
				// index = 3;                             // 반복의 index => 2
				// phoneBook[3] = phoneBook[4]
				// phoneBook[4] = phoneBook[5]            //i<cnt-1 까지
